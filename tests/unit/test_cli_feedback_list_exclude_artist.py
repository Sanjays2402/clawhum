"""--exclude-artist on feedback-list lets the user hide a noisy artist
(covers, alternate editions, smoke-test seeds) from the listing without
rewriting the feedback file."""
...
def test_exclude_artist_whitespace_trimmed(tmp_path, monkeypatch):
    ...
    result = runner.invoke(
        app,
        ["feedback-list", "--exclude-artist", "  QUEEN  ", "--format", "json"],
    )
    assert result.exit_code == 0, result.output
    payload = json.loads(result.stdout)
    ids = {r["track_id"] for r in payload}
...
def test_exclude_artist_keeps_orphans_unless_in_index(tmp_path, monkeypatch):
    _seed(tmp_path, monkeypatch)
    runner = CliRunner()
    # Without --in-index, orphan t4 (no metadata) survives so user can clean it.
    result = runner.invoke(
        app,
        ["feedback-list", "--exclude-artist", "Queen", "--in-index", "--format", "json"],
    )
    assert result.exit_code == 0, result.output
    payload = json.loads(result.stdout)
    ids = {r["track_id"] for r in payload}
