"""`clawhum feedback-list --track-id-file <path>` and `--exclude-track-file
<path>` load track id shortlists/denylists from disk for scoping the listing
when the id set is too long to pass on the command line or is reused across
many feedback-list runs. Blank lines and lines starting with '#' are ignored;
file ids union with any --track-id/--exclude-track values; the existing
...
def test_track_id_file_restricts_to_listed_ids(tmp_path, monkeypatch):
    ...
    shortlist.write_text("# my favourites\nt1\n\nt3\n")
    runner = CliRunner()
    r = runner.invoke(
        app,
        ["feedback-list", "--track-id-file", str(shortlist), "-f", "json"],
    )
    assert r.exit_code == 0, r.output
    assert set(_ids(r.output)) == {"t1", "t3"}
...
def test_track_id_file_unions_with_cli_track_id(tmp_path, monkeypatch):
    ...
    assert r.exit_code == 0, r.output
    assert set(_ids(r.output)) == {"t1", "t2"}
...
def test_exclude_track_file_drops_listed_ids(tmp_path, monkeypatch):
    ...
    r = runner.invoke(
        app,
        ["feedback-list", "--exclude-track-file", str(denylist), "-f", "json"],
    )
    assert r.exit_code == 0, r.output
    ids = set(_ids(r.output))
...
def test_exclude_track_file_unions_with_cli_exclude_track(tmp_path, monkeypatch):
    ...
    assert r.exit_code == 0, r.output
    assert set(_ids(r.output)) == {"t2", "t3"}
...
def test_track_id_file_missing_path_errors(tmp_path, monkeypatch):
    ...
    r = runner.invoke(
        app,
        ["feedback-list", "--track-id-file", str(tmp_path / "nope.txt"), "-f", "json"],
    )
    assert r.exit_code != 0
