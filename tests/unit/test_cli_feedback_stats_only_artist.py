"""--only-artist on feedback-stats restricts the aggregation to one or more
artists. Counterpart to --exclude-artist, modelled on the same flag on the
match command."""
...
def test_only_artist_restricts_to_one_artist(tmp_path, monkeypatch):
    ...
    result = runner.invoke(
        app, ["feedback-stats", "--only-artist", "queen", "--format", "json"]
    )
...
def test_only_artist_whitespace_trimmed(tmp_path, monkeypatch):
    _seed(tmp_path, monkeypatch)
    runner = CliRunner()
    result = runner.invoke(
        app,
        ["feedback-stats", "--only-artist", "  QUEEN  ", "--format", "json"],
    )
    assert result.exit_code == 0, result.output
    payload = json.loads(result.stdout)
    ids = {r["track_id"] for r in payload}
...
def test_no_only_artist_keeps_existing_behaviour(tmp_path, monkeypatch):
    _seed(tmp_path, monkeypatch)
    runner = CliRunner()
