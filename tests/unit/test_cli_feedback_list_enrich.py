"""--enrich joins feedback-list with the indexed library to show
title / artist next to each track_id in recent votes."""
...
def _seed(tmp_path, monkeypatch):
    ...
    write_metadata(meta, [
        ...
        # t3 omitted on purpose so missing tracks degrade to blanks.
    ])

    record_feedback(fb, "q1", "t1", 0.9, 1)
    record_feedback(fb, "q2", "t2", 0.4, -1)
    record_feedback(fb, "q3", "t3", 0.5, 1)  # unknown track
...
def test_feedback_list_enrich_json_adds_title_and_artist(tmp_path, monkeypatch):
    _seed(tmp_path, monkeypatch)
    runner = CliRunner()
    result = runner.invoke(app, ["feedback-list", "--enrich", "--format", "json"])
...
def test_feedback_list_enrich_csv_includes_columns(tmp_path, monkeypatch):
    _seed(tmp_path, monkeypatch)
    runner = CliRunner()
    result = runner.invoke(app, ["feedback-list", "--enrich", "--format", "csv"])
    assert result.exit_code == 0, result.output
...
def test_feedback_list_without_enrich_omits_columns(tmp_path, monkeypatch):
    _seed(tmp_path, monkeypatch)
    runner = CliRunner()
...
def test_feedback_list_enrich_table_renders_human_columns(tmp_path, monkeypatch):
    _seed(tmp_path, monkeypatch)
    from rich.console import Console
    import cli.clawhum_cli.main as cli_main
    monkeypatch.setattr(cli_main, "console", Console(width=200, force_terminal=False))
    runner = CliRunner()
    result = runner.invoke(app, ["feedback-list", "--enrich", "--format", "table"])
    assert result.exit_code == 0, result.output
