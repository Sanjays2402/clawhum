"""--exclude-artist-file on feedback-list loads a persistent deny-list of
noisy artists from disk so users do not have to retype the same
``--exclude-artist X --exclude-artist Y`` list on every listing."""
...
def test_exclude_artist_file_loads_names_and_drops_matches(tmp_path, monkeypatch):
    ...
    result = runner.invoke(
        app,
        ["feedback-list", "--exclude-artist-file", str(deny), "--format", "json"],
    )
...
def test_exclude_artist_file_missing_path_is_rejected(tmp_path, monkeypatch):
    ...
    result = runner.invoke(
        app,
        ["feedback-list", "--exclude-artist-file", str(tmp_path / "nope.txt")],
    )
