...
from .routes import webhook_max_attempts_policy as webhook_max_attempts_policy_routes
...
from .routes import match_topk as match_topk_routes
...
async def _lifespan(app: FastAPI):
    ...
    from . import webhook_secret_rotation as _webhook_secret_rotation
    _webhook_secret_rotation.reset_cache()
    from . import scim_token_rotation as _scim_token_rotation
    ...
    from . import match_topk as _match_topk
...
def create_app() -> FastAPI:
    ...
    # Same edge-position rationale as PAT expiry warning: needs to see
    # the final response so headers land on 4xx and 5xx too.
    app.add_middleware(ScimTokenRotationMiddleware)
    ...
    app.include_router(webhook_max_attempts_policy_routes.router)
    ...
    app.include_router(match_topk_routes.router)
    app.include_router(match_topk_routes.router, prefix="/v1")
    ...
    app.include_router(webhook_max_attempts_policy_routes.router, prefix="/v1")
